...
sources = [Glob("Inworld/src/*.cpp")]

env.Append(CPPPATH=["Inworld/inworld-ndk/src/Public"])
env.Append(CPPPATH=["Inworld/inworld-ndk/src/Private"])
env.Append(CPPPATH=["Inworld/inworld-ndk/src-proto"])
env.Append(CPPPATH=["Inworld/inworld-ndk/include"])
env.Append(CPPPATH=["Inworld/inworld-ndk/ThirdParty/include"])
...
if env["platform"] == "macos":
    ...
    library = env.SharedLibrary(
        "project/addons/inworld/bin/libinworld{}{}".format(
